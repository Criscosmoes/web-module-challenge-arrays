...
console.log(copy(copyOfArray, originalFlavors)); 
...
function filterByWord(arr, strFlavor){

    let filteredArr = []; 

    for(let i = 0; i < arr.length; i++){

        if(arr[i].includes(strFlavor) === true){
...
function getAverageWordLength(arr){
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        sum += arr[i].replace(/[^A-Z]/g, "").length;
        
    }

    let average = (sum/arr.length); 

    console.log(`The average words in this array is ${average.toFixed(2)}`); 
